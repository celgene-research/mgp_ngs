...
sub getDatabaseConnection{
	...
	$logger->info("getDatabaseConnection: Connecting to SOLR on http://$server:$port/solr/oodt");
...
# routine to get the basename and sampleid
sub getSampleIDByBasename{
	my($filename)=@_;
        $logger->info("getSampleIDByBasename: looking for ID for [$filename]");
        ...
        my $query2 = { q => 'Filename:"'.$basename.'"'};
        my $results=_getArrayResults( $solr, $query2, $params);
        $logger->info("getSampleIDByBasename: for [$filename] found samples :[",join(",", @$results),"]");
        $logger->info("getSampleIDByBasename: function call finished SUCCESSFULLY");
