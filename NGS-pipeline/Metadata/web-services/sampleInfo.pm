...
sub createSample{
	...
	my $sql=qq/
		...
		vendor_project_name,
		compound_array,dose_array,time_treatment,xenograft,tissue,condition,condition_array,
		...
		'$inputHash->{condition}','{ "/ . join('","', @{$inputHash->{condition_array}}). qq/"}',
...
sub updateAlignmentQC{
	...
	my $sqlh="update sample_alignmentqc set( ";
		...
		$sqlh.=" $bq->{wgs_mean_coverage}," if defined($bq->{ wgs_mean_coverage  } );   
 		...
 		$sqlh.=" $bq->{wgs_pct_20X}," if defined($bq->{ wgs_pct_20X } );
 		$sqlh.=" $bq->{wgs_pct_25X}," if defined($bq->{ wgs_pct_25X } );
 		$sqlh.=" $bq->{wgs_pct_30X}," if defined($bq->{ wgs_pct_30X } );
 		$sqlh.=" $bq->{wgs_pct_40X}," if defined($bq->{ wgs_pct_40X } );
 		$sqlh.=" $bq->{wgs_pct_50X}," if defined($bq->{ wgs_pct_50X } );
 		$sqlh.=" $bq->{wgs_pct_60X}," if defined($bq->{ wgs_pct_60X } );
 		$sqlh.=" $bq->{wgs_pct_70X}," if defined($bq->{ wgs_pct_70X } );
...
sub getQCReportTable{
	...
	sa.pf_reads_aligned as reads_aligned,
	...
	left join sample_alignmentqc sa on sa.sample_id=si.sample_id and sa.flag='$flag'
	left join sample_readqc sr on sr.sample_id=si.sample_id and sr.flag='$flag'
	join sample_experiment se on si.sample_id=se.sample_id
