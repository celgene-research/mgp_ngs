...
my $methods = {	
				...
				'metadataInfo.getSampleIDByBasename'=>\&metadataInfo::getSampleIDByBasename,
...
$ENV{ POSTGRES_SERVER_IP }=$configuration->param('postgres_server_ip');
$ENV{ POSTGRES_SERVER_PORT }=$configuration->param('postgres_server_port');
...
if(!defined($s)){
$logger->logdie("Cannot establish connection to SOLR server");
}
