...
my $svnversion=Celgene::Utils::SVNversion::version( '$Date: 2015-06-04 11:34:19 -0700 (Thu, 04 Jun 2015) $ $Revision: 1581 $' );
my @allArgs=@ARGV;
...
sub printVersion{
	print "filterBamAlnQuality.pl  $svnversion \n";
}
...
while (my $line=<$inFh>){
	...
	$score->{ $mate }->{ mapq }=$mapq if (defined($mapq) and defined($score->{ $mate }->{ mapq } ) and $mapq > $score->{ $mate }->{ mapq });
	$score->{ $mate }->{ as   }=$alnScore if (defined($alnScore) and defined($score->{ $mate }->{ as   }) and $alnScore > $score->{ $mate }->{ as });
...
#	}
	...
	foreach my $a(@$readData){
		...
		my $mate;
		# output unmapped reads and continue
		if( $flag & 4 ){
			print $outFh $a."\n";
			next;
		}
		...
		if( defined($score->{$mate}->{mapq}) and $score->{$mate}->{mapq} >0){
			...
			if ( defined( $score->{$mate}->{as}) and $alnScore < $score->{$mate}->{as}){ 
				$stats{$mate}++;
				$dropread=1;
				next;}
		}
...
sub setUpLog{
	...
	if(!defined($logFile)){$logFile=$v."/filterBamAlnQuality.log";}
	my $logConf=qq{
	...
	my $logger=Log::Log4perl->get_logger("filterBamAlnQuality");
	return $logger;
}
...
sub printHeader{
	...
	# add a line with the information of filterBamAlnQuality
	print $outFh "\@CO\tuser command line filterBamAlnQuality: $0 @allArgs\n";
}
