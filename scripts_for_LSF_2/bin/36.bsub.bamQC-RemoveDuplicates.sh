...
input=$1
echo "This script uses the picard tools MarkDuplicates to generate a file without any duplicate reads"
echo "Use for ChIP-Seq experiments where duplicates are not needed"

checkfile $input

analysistask=60
step="RemoveDuplicates"
...
newDir=\$( lastDir \$input | sed 's%bamfiles%bamfiles-Rmdup%')
...
java -Xmx${memory}m -jar ${PICARDBASE}/picard.jar MarkDuplicates \
VERBOSITY=WARNING \
INPUT=\$input \
TMP_DIR=\${NGS_TMP_DIR} \
M=\${outputDirectory}/${stem}.${step}.qcstats \
CREATE_INDEX=TRUE AS=TRUE O=\${outputDirectoryBam}/${stem}.${step}.mdup.bam \
