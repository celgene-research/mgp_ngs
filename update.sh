...
for i in `find $DD/NGS-pipeline/  | grep pl$`; do chmod 755 $i; ln -sf /celgene/software/bin/$(basename $i) $i ; done
for i in `find $DD/NGS-pipeline/  | grep sh$`; do chmod 755 $i; ln -sf /celgene/software/bin/$(basename $i) $i  ; done
for i in `find $DD/NGS-pipeline/  | grep R$`; do chmod 755 $i; done
for i in `find $DD/oodt/  | grep sh$`; do chmod 755 $i; ln -sf /celgene/sofware/bin/$(basename $i) $i ; done