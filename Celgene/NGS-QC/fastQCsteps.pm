...
sub _loadAdapters{
	...
	my $inFn=FileFunc::newReadFileHandle( "$FindBin::RealBin/adapters/$filename");
...
sub parseFastQC{
	my($self, $file)=@_;
	$self->{rfh}=FileFunc::newReadFileHandle( $self->resultsFile( $file ) );
...
sub parseLaneDistribution{
	...
	my $rfh=FileFunc::newReadFileHandle(   $self->outputFile() );
