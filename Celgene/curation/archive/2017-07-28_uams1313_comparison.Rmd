...
u1313 <- s3_get_csv("Curated_Data_Sources/2017-07-26_1313_UAMS_WES_Cohort.csv") %>%
  mutate(set = "U1313")

# Import current metadata and MMRF QC tables to facilitate annotation of excluded patients in u1313 cohort
...
bind_rows(lapply(list(mgp.wes, u1313), extract)) %>%
  mutate(c = Study) %>%
...
```{r, echo=F}
venn::venn(list(`MGP WES` = mgp.wes$Patient, 
                `UAMS 1313`    = u1313$Patient), 
           cexsn = 1, cexil = 1, zcolor = "style")

fourty <- u1313[!u1313$Patient %in% mgp.wes$Patient,]
fourty$Patient
```

\

In addition to changes in PFS and OS durations, 40 patients in the u1313 dataset were previously excluded from the MGP cohort.

- 20 DFCI patients were removed after reclassified as Relapse 
- 16 MMRF patients are classified as PCL during their diagnosis sample.
- 4 MMRF patient samples were recently flagged by the MMRF due to QC issues, replacement files are available in IA10 but have not been processed as of yet.
