...
###################################################################
#############   One-time fix missing MMRF metadata  ###############
###################################################################
#

# incomplete <- dts$metadata %>% filter( is.na(Study) )
# 
# updated <- GetS3Table(file.path(s3, "ClinicalData/ProcessedData/MMRF_IA10c/curated_MMRF_CoMMpass_IA10_Seq_QC_Summary.txt")) %>%
#   filter(File_Name %in% incomplete$File_Name) %>%
#   select(File_Name, Study, Sample_Name, Visit_Name, 
#          Disease_Status, Sequencing_Type, Excluded_Flag) %>%
#   mutate(Tissue_Type  = gsub(".*_([BMP]+)_.*", "\\1", File_Name),
#          Cell_Type    = gsub(".{12}[PBM]+_([A-Za-z0-9]+)_[CT]\\d.*","\\1",File_Name),
#          Cell_Type    = gsub("WBC|Whole", "PBMC", Cell_Type),
#          Sample_Type_Flag = if_else(Cell_Type == "CD138pos" & Tissue_Type == "BM", 1, 0),
#          Sample_Type  = recode(Sample_Type_Flag, "1"="NotNormal", "0"="Normal"),
#          Disease_Type = "MM") %>%
#   append_df(dts$metadata, ., id = "File_Name", mode = "replace")
# 
# PutS3Table(updated, file.path(s3,"ClinicalData/ProcessedData/JointData/curated_metadata_2017-04-17.txt" ))
# 
# dim(dts$metadata) - dim(updated)
# dts$metadata <- updated
