...
df <- read.delim("file_inventory.txt", stringsAsFactors = F)

#uams needs to be curated before using as lookup table
# uams_lookup <- read.delim()
# append_df(df, uams_lookup[,c("Sample_name", "Patient")], id = "SampleName")
